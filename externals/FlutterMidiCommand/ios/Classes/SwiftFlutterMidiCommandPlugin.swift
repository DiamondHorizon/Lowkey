...
public class SwiftFlutterMidiCommandPlugin: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate, FlutterPlugin {
    ...
    public func centralManager(_ central: CBCentralManager, didConnect peripheral: CBPeripheral) {
        sendLogToDart("Successfully connected to peripheral: \(peripheral.name ?? "Unnamed")") // Copilot
        print("central did connect \(peripheral)")

        // Copilot
        guard let device = connectedDevices[peripheral.identifier.uuidString] as? ConnectedBLEDevice else {
            sendLogToDart("Connected peripheral not found or not a BLE device.")
            return
        }
